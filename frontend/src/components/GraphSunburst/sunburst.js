...
export default Kapsule({
  props: {
    width: { default: Math.min(window.innerWidth, window.innerHeight) * 0.85 },
    height: { default: Math.min(window.innerWidth, window.innerHeight) * 0.85 },
    data: {
      onChange() {
        this._parseData()
      },
  ...
  methods: {
    _parseData(state) {
      if (state.data) {
        ...
        ).eachAfter((n) => (n.value = n.data.size))
  ...
  init(domNode, state) {
    ...
    state.canvas.on('mousemove', () => {
      state.tooltip
        ...
        .style(
          'transform',
          `translate(-${(d3Event.offsetX / state.width) * 100}%, 21px)`
  ...
  update(state) {
    ...
    state.radiusScale.range([maxRadius * 0.1, maxRadius])
    ...
    state.svg
      ...
      .attr(
        'viewBox',
        `${-state.width / 2} ${-state.height / 2} ${state.width} ${
          state.height
        }`
      )
    ...
    const focusD = (state.focusOnNode && state.focusOnNode.__dataNode) || {
      x0: 0,
      x1: 1,
      y0: 0,
      y1: 1,
    ...
    const slice = state.canvas.selectAll('.slice').data(
      state.layoutData.filter(
        (
          d // Show only slices with a large enough angle
        ) =>
          d.x1 >= focusD.x0 &&
          d.x0 <= focusD.x1 &&
    ...
    const newSlice = slice
      ...
      .on('mouseover', (d) => {
        ...
        state.tooltip.html(
          `<div class="tooltip-title">${getNodeStack(d)
            .map((d) => nameOf(d.data))
    ...
    const label = newSlice.append('text').attr('class', 'path-label')
    ...
    allSlices
      ...
      .styleTween('display', (d) => () =>
        state.showLabels && textFits(d) ? null : 'none'
      )
