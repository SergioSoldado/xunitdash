...
import md5 from '~/thirdparty/js/md5.min'
import range from '~/thirdparty/js/range'

const Identicon = (props) => {
  let { fg, bg, count, palette, string, size, getColor, padding, className } = props
  ...
  const updateCanvas = () => {
    ...
    if (palette && palette.length) {
      let index = Math.floor(range(parseInt(hash.slice(-3), 16), 0, 4095, 0, palette.length))
      fg = palette[index]
    }
